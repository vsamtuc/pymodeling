...
#
# An implementation of the C3 algorithm for class linearization
#

def c3_merge(seqs):
	"""Merge the given sequences according to the C3 linearization
	algorithm.

	This code can be used to generate the MRO (Member Resolution Order)
	of python classes.
	"""

	#
	# Adapted from code by Samuele Pedroni
	# 
	assert isinstance(seqs,list) and \
		all(isinstance(s, list) for s in seqs)

	def find_candidate():
		nonlocal seqs
		for s in seqs:
			c = s[0]
			if all(c not in q[1:] for q in seqs):
				return c
		raise ValueError("Could not linearize classes", seqs)
	...
	# remove any empty lists from seqs
	remove_empty_lists()
...
class Class(NamedEntity):
	...
	The model supports multiple inheritance and has no 'Object' class.
	The semantics is much like C++ virtual inheritance. 
	Also, we use C3 linearization (also used by python3) to determine
	the Method Resolution Order (MRO) of superclasses.
	"""
	def __init__(self, name=None, superclasses=None, attributes=set()):
		...
		# costruct the Member Resolution Order
		#(1) compute the iterative lookup list
		lin = c3_merge([[self]]+
			[list(c.mro) for c in self.__superclasses]+
			[sclist])

		self.__mro = tuple(lin)
	...
	def superclasses(self):
		"""Return the tuple of *direct* superclasses of this class"""
		return self.__superclasses
	
	@property
	def mro(self):
		"""
		The member resolution order (mro) for this class. 

		The mro is an immutable sequence of all superclasses (including
		self), which is computed by the C3 linearization algorithm.
		"""
		return self.__mro
	
	def is_subclass(self, other):
		"""Return True if this class is a subclass of ``other``."""
		if not isinstance(other, Class):
			raise TypeError("An mf.Class was expected")
		return other in self.__mro
		
	def is_superclass(self, other):
		"""Return True if this class is a superclass of ``other``."""
		return other.is_subclass(self)
	
	# for naming compatibility
	all_superclasses = mro
		
	def all_proper_superclasses(self):
		"""Return an iterable of all proper superclasses (excluding self)."""
		return self.__mro[1:]    
	...
	def all_attributes(self):
		...
		"""
		for c in self.all_superclasses:
			yield from c.attributes
	...
	def get_relationship(self, name):
		"""Get a relationship by name or raise a KeyError.
		"""
		for c in self.mro:
...
def extract_model(cls):
	...
	# get superclass
	sc = [bc.__model_class__  for bc in cls.__bases__ 
				if hasattr(bc, '__model_class__')]
...
class ClassValidation(Validation):
	...
	def validate_model_class(self, cls, validset):
		# We recursively validate the class hierarchy
		# of our superclasses
		...
		# validate superclasses if needed
		for sc in cls.superclasses:
			isvalid = self.validate_model_class(sc, validset)
		...
		with self.section("Class '{0}'", cls.name) as V:            
			...
			if validset is not None:
				for sc in cls.superclasses:
					V(validset[sc], "Superclass is valid")
...
class _CORE:
	...
	mClass.add_attribute(name='superclasses', type=tuple, default=None)
